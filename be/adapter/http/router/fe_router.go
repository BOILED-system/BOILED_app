...
func SetupFE(mux *http.ServeMux, feHandler *handler.FEHandler) {
	...
	mux.HandleFunc("PUT /api/users/{memberId}", feHandler.UpdateUser)
	mux.HandleFunc("DELETE /api/users/{memberId}", feHandler.DeleteUser)
	...
	mux.HandleFunc("GET /api/members/{memberId}/payments", feHandler.GetMyPayments)
