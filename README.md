...
- Node.js 18+
- Go 1.22+
- GCPプロジェクト (`boiled-app-bb43e`) へのアクセス権限（Firestore読み書き）

### バックエンド起動

```bash
cd be
export GCP_PROJECT_ID=boiled-app-bb43e
...
npm install
npm run dev
# -> http://localhost:3000 で起動（デフォルトでhttp://localhost:8080のAPIへ接続）
```
...
```bash
# プロジェクトルートに .env を作成
GCP_PROJECT_ID=boiled-app-bb43e
...
Firestoreコンソール（https://console.firebase.google.com/）の `users` コレクションにドキュメントを追加する。
